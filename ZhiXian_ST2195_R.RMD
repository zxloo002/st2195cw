...
arrival_delay <- delayed %>% 
  filter(ArrDelay > 0) %>%
  select(Year, Month, DayofMonth, DayOfWeek, CRSDepTime, CRSArrTime, TailNum, DepDelay, ArrDelay, DepInterval)
...
pdata %>%
  ...
  geom_point() +
  labs(title = "Probability of flight delay", x = "Age of plane", y = "") +
  theme_classic()
```
...
```{r}
delayed$Origin <- as_factor(delayed$Origin)
delayed %<>%
  mutate(Delayed = as_factor(ifelse(ArrDelay > 0 | DepDelay > 0, 1, 0)))

delayed %<>% filter(!is.na(Delayed))
