...
delayed %>%
  ...
  summarize(Percent = round(mean(ADelay == 1) * 100, 2)) %>%
  ggplot(aes(x= Percent, y= DepInterval)) +
  geom_col() +
  labs(title = "Probability of delayed flight", x = "Percentage", y = "") +
  scale_y_discrete(limits = rev) +
  geom_text(aes(label = Percent), hjust = - 0.1 ,position = position_dodge(width = 1), size = 3) +
  theme_classic()
...
delayed %>%
  ...
  group_by(DayOfWeek) %>% 
  summarize(Percent = round(mean(ADelay == 1) * 100, 2)) %>%
  ggplot(aes(x= Percent, y= DayOfWeek)) +
  geom_col() +
  labs(title = "Probability of delayed flight", x = "Percentage", y = "") +
  scale_y_discrete(limits = rev) +
  geom_text(aes(label = Percent), hjust = - 0.1 ,position = position_dodge(width = 1), size = 3) +
...
pdata %>%
  #mutate(ManYear = as_factor(ManYear)) %>%
  filter(ManYear != 0) %>%
  group_by(ManYear) %>%
  summarise(Percent = round(mean(ADelay == 1 | DDelay == 1)* 100,2)) %>%
  ggplot(aes(x = ManYear, y = Percent)) +
  geom_point() +
  labs(title = "Probability of flight delay", x = "Manufacture Year", y = "") +
...
ydata <- pdata %>% 
  filter(!(DepDelay <= 0 & ArrDelay <= 0)) %>% #filter out flights with no delays
...
delayed$UniqueCarrier <- as.factor(flight$UniqueCarrier)
delayed %<>% na.omit(delayed$Delayed)
...
task$select(c('Year', 'Month', 'DayofMonth', 'DayOfWeek', 'CRSDepTime', 'UniqueCarrier'))
